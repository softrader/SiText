"""Configuration management for SiText."""
...
class Config:
    ...
    DEFAULT_CONFIG = {
        ...
        # AI/ML features for semantic search
        "ai": {
            "enabled": False,  # Disabled by default - enable in settings
            "model": "all-MiniLM-L6-v2",  # sentence-transformers model
            "auto_index": True,  # Auto-index notes in background
        },
    ...
    def __init__(self, config_path: Optional[Path] = None) -> None:
        ...
        Args:
            config_path: Optional path to config file. Defaults to ~/.sitext/config.json
        """
        if config_path is None:
            config_path = Path.home() / ".sitext" / "config.json"
    ...
    def notes_directory(self) -> Path:
        ...
        Checks in order:
        1. Environment variable SITEXT_DIR
        ...
        env_dir = os.environ.get("SITEXT_DIR")
        if env_dir:
